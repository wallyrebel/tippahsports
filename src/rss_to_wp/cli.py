...
def process_entry(
    ...
    # Final Fallback: Local fallback image (e.g., logo)
    if not image_result and feed_config.fallback_image:
        from pathlib import Path
        config_dir = Path(config_path).parent if config_path else Path(".")
        fallback_path = config_dir / feed_config.fallback_image
        if fallback_path.exists():
            logger.info("using_local_fallback_image", path=str(fallback_path))
            with open(fallback_path, "rb") as f:
                image_bytes = f.read()
            filename = fallback_path.name
            image_result = (image_bytes, filename, "image/jpeg")
            image_alt = "Tippah County Sports"
        else:
            logger.warning("fallback_image_not_found", path=str(fallback_path))
